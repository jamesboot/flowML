...
setwd('/Users/jamesboot/Documents/9.Genome Centre Files/Platelet Analysis/WholeBlood_Analysis/')
...
# For loop to loop analysis through all treatments - run each treatment individually
for (treatment in unique(meta_data$Treatment)) {
  
  # Create directory for saving
  dir.create(treatment)
  
  # Subset the loaded data frame to only contain one of the treatments
  # Select rows based on treatment meta
  fcs_dat_sub <- fcs_dat_trans[(meta_data$Treatment == treatment),]
  row.names(fcs_dat_sub)
  
  # Subset the meta
  meta_data_sub <- meta_data[meta_data$Treatment == treatment,]
  ...
  ggsave(paste0(treatment, '/', treatment, '_Cluster.png'),
         height = 2099,
         width = 2099,
         dpi = 300,
         units = 'px')
  ...
  make.pheatmap(exp, 
                "FlowSOM_metacluster", 
                colnames(fcs_dat_sub),
                file.name = paste0(treatment, '/', treatment,'_cluster_heatmap.png'))
  dev.off()
  ...
  ggsave(paste0(treatment, '/', treatment, '_Treatment.png'),
         plot = last_plot(),
         height = 2099,
         width = 2099,
         dpi = 300,
  ...
  ggsave(paste0(treatment, '/', treatment, '_Batch.png'),
         plot = last_plot(),
         height = 2099,
         width = 2099,
         dpi = 300,
  ...
  ggsave(paste0(treatment, '/', treatment, '_Sample.png'),
         plot = last_plot(),
         height = 2099,
         width = 2099,
         dpi = 300,
  ...
  ggsave(paste0(treatment, '/', treatment, '_Donor.png'),
         plot = last_plot(),
         height = 2099,
         width = 2099,
         dpi = 300,
  ...
  # Make a data table of col axis limits for plotting below 
  lims <- data.table(
    "GPVI_logicle" = c(0, 4.5),
    "CD62P_logicle" = c(0, 4.5),
    "CD61_logicle" = c(0, 4.5),
    "CD29_logicle" = c(0, 4.5),
    "CD9_logicle" = c(0, 4.5),
    "CD36_logicle" = c(0, 4.5),
    "CD42b_logicle" = c(0, 4.5),
    "CLEC-2_logicle" = c(0, 4.5),
    "CD31_logicle" = c(0, 4.5),
    "PAC-1_logicle" = c(0, 4.5),
    "CD107a_logicle" = c(0, 4.5),
    "CD63_logicle" = c(0, 4.5),
    "CD42a_logicle" = c(0, 4.5),
    "CD41_logicle" = c(0, 4.5),
    "FSC-A_logicle" = c(0, 4.5),
    "SSC-A_logicle" = c(0, 4.5)
  )
  
  # For loop to plot all genes
  for (x in colnames(fcs_dat_sub)) {
    ...
    ggsave(paste0(treatment, '/', treatment, '_', x, '.png'),
           plot = last_plot(),
           height = 2099,
           width = 2099,
           dpi = 300,
  ...
  post_meta <- cyt[, c('Treatment', 'FlowSOM_metacluster', 'Donor', 'Batch')]
  ...
  ggsave(plot = last_plot(),
         filename =  paste0(treatment, '/', treatment, '_Donor_Distr.png'),
         height = 2099,
         width = 2099,
         dpi = 300,
         units = 'px')
  ...
  # Plot summary stacked bar chart
  ggplot(summary, aes(x = Cluster, y = n, fill = Batch)) +
    geom_bar(stat = "identity", position = 'stack') +
  ...
  ggsave(plot = last_plot(),
         filename =  paste0(treatment, '/', treatment, '_Batch_Distr.png'),
         height = 2099,
         width = 2099,
         dpi = 300,
         units = 'px')
...
# Subset down to the selected treatments - Vehicle plus one Agonist
# For both data and metadata
data_new <- fcs_dat_trans[meta_data$Treatment %in% c('VEH', 'U4'), ]
meta_new <- meta_data[meta_data$Treatment %in% c('VEH', 'U4'), ]
...
sum(dataTraining$identity == 'VEH')
sum(dataTraining$identity == 'U4')
sum(dataUnseen$identity == 'VEH')
